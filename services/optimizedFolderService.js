...
class OptimizedFolderService {
    ...
    async getDirectoryStats(directory, includeSubfolders = true) {
        try {
            ...
            const allFiles = [];
            const allFolders = [];
            await this._getStatsRecursiveOptimized(directory, stats, allFiles, allFolders, includeSubfolders, 0);
            ...
            // 计算文件夹大小并排序最大文件夹
            const foldersWithSize = await this._calculateFolderSizes(allFolders);
            stats.largestFolders = foldersWithSize
                .sort((a, b) => b.size - a.size)
                .slice(0, 10);
    ...
    // 优化版递归统计
    async _getStatsRecursiveOptimized(dir, stats, allFiles, allFolders, includeSubfolders = true, currentDepth = 0) {
        try {
            const entries = await fs.readdir(dir, { withFileTypes: true });
            ...
            if (includeSubfolders) {
                for (const dirEntry of directories) {
                    const dirPath = path.join(dir, dirEntry.name);
                    await this._getStatsRecursiveOptimized(dirPath, stats, allFiles, allFolders, includeSubfolders, currentDepth + 1);
                }
            }
    ...
    // 计算文件夹大小
    async _calculateFolderSizes(folders) {
        const foldersWithSize = [];

        for (const folder of folders) {
            try {
                const size = await this._getFolderSize(folder.path);
                foldersWithSize.push({
                    ...folder,
                    size: size
                });
            } catch (error) {
                console.error(`计算文件夹大小失败 ${folder.path}:`, error);
                foldersWithSize.push({
                    ...folder,
                    size: 0
                });
        ...
        try {
            ...
            for (const entry of entries) {
                ...
                if (entry.isFile()) {
                    ...
                    totalSize += await this._getFolderSize(entryPath);
                }
            }
        } catch (error) {
            // 忽略无法访问的目录
